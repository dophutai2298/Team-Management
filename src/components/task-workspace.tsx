...
export function TaskWorkspace() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1320px]">
      ...
      <TaskDetailModal error={viewingTaskQuery.isError ? errorMessage(viewingTaskQuery.error, t("tasks.errorGeneric")) : undefined} isLoading={viewingTaskQuery.isPending} isOpen={Boolean(viewingTaskId)} task={viewingTask} onClose={() => setViewingTaskId(null)} onDelete={() => viewingTask && setDeleteTaskId(viewingTask.id)} onEdit={() => viewingTask && openTaskEditor(viewingTask.id)} onTaskChange={(task) => queryClient.setQueryData(["tasks", task.id], { task })} onUpdateProgress={() => viewingTask && openTaskProgress(viewingTask.id)} />
